...
class Module(nn.Module):
    ...
    def save(self):
        values = {name: param.detach().cpu().numpy() for name, param in self.named_parameters()}
        amount = len(values)
    ...
    def load(self, values):
        ...
        for name, param in self.named_parameters():
            if name in values:
                param.data.copy_(torch.tensor(values[name], dtype=param.dtype, device=param.device))

    def get(self, name, ctor, *args, **kwargs):
        """Retrieve a cached submodule (creating it if necessary)."""
        if not hasattr(self, '_modules_dict'):
            self._modules_dict = {}
        if name not in self._modules_dict:
            if 'name' in inspect.signature(ctor).parameters:
                kwargs['name'] = name
            mod = ctor(*args, **kwargs)
            self._modules_dict[name] = mod
...
class AutoAdapt(Module):
    def __init__(self, shape, impl, scale, target, min_val, max_val, vel=0.1, thres=0.1, inverse=False):
        super().__init__()
        self._shape = shape
        self._impl = impl
        self._target = target
